...
bool Pipeline::Create(VkRenderPass aPass, const char* aName /*= 0*/) {
	...
	VkPipelineLayoutCreateInfo pipelineLayoutInfo = {};
	{
		...
		//const size_t numPcs = mPushConstants.size();
		//std::vector<VkPushConstantRange> pushConstants(numPcs + mBindlessTextures);
		//for(int i = 0; i < numPcs; i++) {
		//	pushConstants[i] = mPushConstants[i];
		//}
		//if(mBindlessTextures) {
		//	VkPushConstantRange globalTexturePCRange {};
		//	globalTexturePCRange.offset = 0;
		//	globalTexturePCRange.size = sizeof(Graphics::GlobalTexturePC);
		//	globalTexturePCRange.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
		...
		std::vector<VkDescriptorSetLayout> layouts(numMaterials + mBindlessTextures);
